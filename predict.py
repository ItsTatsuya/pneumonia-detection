...
def parse_args():
    ...
    parser.add_argument('--threshold', type=float, default=None,
                        help='Classification threshold override (if None, uses threshold_file or 0.5)')
    parser.add_argument('--threshold_file', type=str, default=None,
                        help='Path to threshold JSON generated by training (validation-selected threshold)')
    parser.add_argument('--optimize_threshold', action='store_true', default=False,
                        help='Optimize threshold on current dataset labels (analysis only; not holdout-safe)')
...
def main():
    ...
    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
        ...
        print(
            ...
            f"(val_acc={checkpoint.get('best_val_acc', 0.0):.4f}, "
            f"val_f1={checkpoint.get('best_val_f1', 0.0):.4f}, "
            f"val_auc={checkpoint.get('best_val_auc', 0.0):.4f})"
        )
    else:
    ...
    # 2) --threshold_file artifact from training (validation-selected)
    # 3) --optimize_threshold on current labeled set (analysis only)
    # 4) fallback 0.5
    threshold = 0.5

    if args.threshold is not None:
        threshold = args.threshold
        print(f"Using explicit threshold override: {threshold:.3f}")
    elif args.threshold_file:
        with open(args.threshold_file, 'r') as f:
            threshold_payload = json.load(f)
        threshold = float(threshold_payload['threshold'])
        print(f"Loaded validation-selected threshold from file: {threshold:.3f}")

    # Find optimal threshold on the current dataset labels only if explicitly requested
    if args.optimize_threshold:
        optimal_threshold, threshold_df = find_optimal_threshold(numeric_labels, probabilities, metric='combined')
        threshold = optimal_threshold
        print(
            "\nOptimal threshold found on current dataset labels: "
            f"{optimal_threshold:.3f} (analysis mode; avoid for strict holdout reporting)"
        )
